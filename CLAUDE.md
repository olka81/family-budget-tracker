...
12. **`ConfigureApplicationCookie` must be called after `AddIdentity(...)`** to point the cookie-authentication challenge at the actual scaffolded Area paths: `options.LoginPath = "/Identity/Account/Login"; options.LogoutPath = "/Identity/Account/Logout"; options.AccessDeniedPath = "/Identity/Account/Login";` (no dedicated `AccessDenied` page exists, so it points at Login too, for now). Without this, ASP.NET Core Identity's cookie middleware falls back to its own default `/Account/Login` — a path that doesn't exist in this project (real pages live under the `Identity` Area) — so an unauthenticated user hitting an `[Authorize]`-protected page gets redirected to a 404 (rendered via `Router.NotFoundPage`) instead of the real login page. This is separate from Decision #6's Razor-Pages-vs-Blazor-components reasoning; it only affects *where* the automatic redirect points, not which technology renders the login page.
...
**Main Blazor app now integrated with Identity, verified end-to-end** (this was open finding (2) below — now resolved, not deferred): `Components/Routes.razor` uses `AuthorizeRouteView` instead of `RouteView` (requires `@using Microsoft.AspNetCore.Components.Authorization` in `_Imports.razor`). `Components/Layout/NavMenu.razor` has an `<AuthorizeView>` block showing "Hello, {email}!" + a Logout form (using the built-in `<AntiforgeryToken />` component, posting to `Identity/Account/Logout`) when signed in, or Register/Login links when not — this also gave `AddCascadingAuthenticationState()` (registered since early in Этап 5, previously unused) its first real consumer. `_Imports.razor` needs *both* `Microsoft.AspNetCore.Components.Authorization` (Blazor's `AuthorizeView`/`AuthorizeRouteView`/`AntiforgeryToken`) and the separate `Microsoft.AspNetCore.Authorization` (the `[Authorize]` attribute itself) — same two-similarly-named-namespaces trap as Decision #11's `IEmailSender`. `Components/Pages/Counter.razor` has `@attribute [Authorize]` added as a **deliberate, permanent test page** (not temporary) to keep a live example of a protected route until real protected pages (purchases, reports) exist. Verified working: an unauthenticated user hitting `/counter` is redirected to `/Identity/Account/Login?ReturnUrl=%2Fcounter` (see Architecture Decision #12) and lands back on Counter after logging in; an authenticated user sees Counter directly and sees "Hello, {email}!"/Logout in the nav.

One finding from the general code review remains open: **deliberately deferred** — `Login.cshtml` has two unconditional dead links (`ForgotPassword`, `ResendEmailConfirmation`) to pages that were never scaffolded; known, low-risk (cosmetic, isolated, doesn't compound over time), to be fixed whenever convenient, not urgent.
