...
Two findings from the general code review: (1) **deliberately deferred** — `Login.cshtml` has two unconditional dead links (`ForgotPassword`, `ResendEmailConfirmation`) to pages that were never scaffolded; known, low-risk (cosmetic, isolated, doesn't compound over time), to be fixed whenever convenient, not urgent. (2) **not yet decided** — the main Blazor app (`Components/`) has no Identity integration yet — no `[Authorize]`, no login/logout link in the nav, no consumer of `AuthenticationStateProvider`.
